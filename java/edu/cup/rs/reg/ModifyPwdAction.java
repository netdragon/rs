...
{
	protected static final LogHandler logger=LogHandler.getInstance(ModifyPwdAction.class);
	...
	protected void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try{
            String oldPwd = BaseFunction.null2value(request.getParameter("txtpwdold"));
            String password=BaseFunction.null2value(request.getParameter("txtpwdnew"));
            ...
            try {
                ...
                if(!user.getUserpwd().equals(Encoder.encrypt(oldPwd))){
					response.sendRedirect("error.jsp?error="+new UTF8String("输入的原密码不正确！").toUTF8String());
					return;
				}
				dbo.update(icl.modifyPwd(Encoder.encrypt(password),Integer.parseInt(USERID)));
