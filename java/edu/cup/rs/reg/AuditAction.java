...
public class AuditAction extends BaseAction
{
	...
	protected static final LogHandler logger=LogHandler.getInstance(AuditAction.class);
	public AuditAction() {
		super();
	}
	...
	protected void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try{
			...
			if(null!=request.getParameterValues("chk_bmxx")) sa_bmxx=request.getParameterValues("chk_bmxx");
                ...
                response.sendRedirect("/error.jsp?error=" + new UTF8String("请选择初审对象!").toUTF8String());
    			return;
			}
            ...
            try {
				bl = new BmxxList();
				dbo.update(bl.audit(bmxxs.toString(), pass));
				ICommonList logslist;
				Log log = new Log();
				logslist = new LogsList();
				log.setContent(USERNAME + " 进行考生初审。");
            ...
            response.sendRedirect("admin/chushen.jsp?"+param.toString());
            return;
        }
        catch(Exception e)
        {
