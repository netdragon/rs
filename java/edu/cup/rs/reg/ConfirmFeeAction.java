...
public class ConfirmFeeAction extends BaseAction
{

	protected static final LogHandler logger=LogHandler.getInstance(ConfirmFeeAction.class);
	public ConfirmFeeAction() {
		super();
	}
	...
	protected void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try{
			...
			String[] sa_bmxx={""};
			if(null!=request.getParameterValues("chk_bmxx")) sa_bmxx=request.getParameterValues("chk_bmxx");
			else {
            ...
            {
                if(bmxxs.length()==1) bmxxs.append(sa_bmxx[i]);
                else {
					bmxxs.append(",");
            ...
            try {
				bl = new BmxxList();
				dbo.update(bl.confirmFee(bmxxs.toString(), pass));
				ICommonList logslist;
				Log log = new Log();
				logslist = new LogsList();
				log.setContent(USERNAME + " 确认考生交费。");
            ...
            response.sendRedirect("admin/qrbmf.jsp?"+param.toString());
            return;
        }
        catch(Exception e)
        {
