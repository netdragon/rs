...
{
	protected static final LogHandler logger=LogHandler.getInstance(AddCjjdAction.class);
	...
	protected void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try{
			...
			*/
            ...
            if(0==ksZxtdz.length())
            {
                response.sendRedirect("error.jsp?error="+new UTF8String("中学通信地址不能为空!").toUTF8String());
                return;
            }
            ...
            if(0==ksNjfzr.length())
            {
                response.sendRedirect("error.jsp?error="+new UTF8String("年级负责人不能为空!").toUTF8String());
                return;
            }
...
//            response.sendRedirect("loginin/browsecjjd.jsp");
        ...
        catch(Exception e)
        {
            ...
            response.sendRedirect("error.jsp?error=" + new UTF8String("填写成绩鉴定表时发生错误！").toUTF8String());
            return;
        }
