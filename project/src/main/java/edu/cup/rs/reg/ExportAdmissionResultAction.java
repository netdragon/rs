...
public class ExportAdmissionResultAction extends BaseAction {
	protected static final LogHandler logger = LogHandler
			.getInstance(ExportAdmissionResultAction.class);
	...
	static {
		...
		TEMPLATE = hm_env.get("RS_HOME").trim()
				+ "\\data\\appfiles\\template\\zhkzh_template.xls";
	...
	protected void execute(HttpServletRequest request,
		...
		try {
			...
			response.setHeader("Content-disposition", "filename=ks_zhkzh_"
					+ tempName + ".xls");
	...
	public static void saveExcel(String template, String fName)
		...
		try {
			...
			while (rs.next()) {
				...
				/*
				 * if(tjf == 1) columnValue = "是"; else columnValue = "否";
				 */
			}
			copy.write();
		} catch (Exception e) {
