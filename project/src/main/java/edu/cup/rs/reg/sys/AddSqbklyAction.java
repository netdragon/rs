...
public class AddSqbklyAction extends BaseAction {
	...
	protected static final LogHandler logger = LogHandler
			.getInstance(AddSqbklyAction.class);
	...
	protected void execute(HttpServletRequest request,
		...
		try {
			...
			if (0 == mc.length()) {
				response.sendRedirect("error.jsp?error="
						+ new UTF8String("请输入面试分组！").toUTF8String());
				return;
			}
			try {
				dbo.init(false);
			...
			try {
				...
				if (c_curr.after(cl) && c_curr.before(c2)) {
					response.sendRedirect("/error.jsp?error="
							+ new UTF8String("在本期报名时间内不能修改面试分组设置，否则会引起数据不一致!")
									.toUTF8String());
					return;
				}
			...
			response.sendRedirect("admin/sqlywh.jsp?info="
					+ new UTF8String("添加成功！").toUTF8String());
			return;
		} catch (Exception e) {
