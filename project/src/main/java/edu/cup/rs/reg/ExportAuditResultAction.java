...
public class ExportAuditResultAction extends BaseAction {
	protected static final LogHandler logger = LogHandler
			.getInstance(ExportAuditResultAction.class);
	...
	static {
		...
		TEMPLATE = hm_env.get("RS_HOME").trim()
				+ "\\data\\appfiles\\template\\shenhe_template.xls";
	...
	protected void execute(HttpServletRequest request,
		...
		try {
			...
			response.setHeader("Content-disposition", "filename=ks_shenhe_"
					+ tempName + ".xls");
	...
	public static void saveExcel(String template, String fName)
		...
		try {
			...
			rs = dbo.query("select a.bmxxid,ksxm,ksxb,shfzh,wyyz,jg,mz,zzmm,kskl,txdz,shxr,yzbm,ksshji,ksah,c.mc as sqly,zxmc,b.zxtxdz,b.zxybm,b.zxlxdh,fmxm,fmgzdw,fmyddh, "
					+ "mmxm,mmgzdw,mmyddh from bmxx a left join cjjd b on a.bmxxid=b.bmxxid left join sqbkly c on a.sqly=c.id order by a.bmxxid");
			...
			while (rs.next()) {
				...
				sheet.addCell(label);
				k++;
				rs_cj = dbo.query("select shhqk,shhyj from bmxx where bmxxid="
						+ bmxxid);
				if (null == rsmd_shqk) {
