...
public class BmxxList implements ICommonList {
	...
	public void setOrder(String colName) {
		...
		if (colName.indexOf("ksxm") != -1) {
			this.orderColName = this.orderColName.replaceAll("ksxm",
					" convert(ksxm using gbk)");
		}
		if (colName.indexOf("jg") != -1) {
			this.orderColName = this.orderColName.replaceAll("jg",
					" convert(jg using gbk)");
		}
		logger.debug("\n" + this.orderColName);
	}
	...
	public String setAdjustAdmit(String bmxxid, int zyid) {
		return "update bmxx, (select zymc from zhshzy where zyid="
				+ zyid
				+ ") temp set bmxx.sflq=1, bmxx.lqzy=temp.zymc where shhqk=1 and zhkzhid is not null and bmxxid="
				+ bmxxid;
	}
	...
	public String getCount() {
		if (this.xmFilter.length() == 0)
			...
			return "select count(bmxxid) as count from bmxx where ksxm like '%"
					+ this.xmFilter + "%'";
	}
		...
		if (0 == this.bmxxId) {
			if ("-1".equals(this.userid)) {
				if (xmFilter.length() > 0)
					return "select bmxxid,jg,ksxm,ksxb,shfzh,shhqk,shhyj,sfjbmf,zhkzhid,zxmc,kskl,sflq,lqzy from bmxx where ksxm like '%"
							+ this.xmFilter
							+ "%' order by "
							+ this.orderColName
							+ " limit "
					...
					return "select bmxxid,jg,ksxm,ksxb,shfzh,shhqk,shhyj,sfjbmf,zhkzhid,zxmc,kskl,sflq,lqzy from bmxx order by "
							+ this.orderColName
							+ " limit "
							+ this.count
							+ " offset " + this.start;
	...
	public String setSumScore(String bmxxid, float sumScore) {
		return "update bmxx set zongfen=" + sumScore + " where bmxxid="
				+ bmxxid;
	}
	...
	public String createAdmission(int bmxxid, String admNum) {
		return "update bmxx set zhkzhid='" + admNum + "' where bmxxid="
				+ bmxxid;
	}

	public String getAuditResult() {
		if (xmFilter.length() > 0)
			return "select bmxxid,ksxm,jg,ksxb,shfzh,zhkzhid,zongfen,zxmc,kskl,sflq,lqzy,sfjbmf from bmxx where shhqk=1 and ksxm like '%"
					+ this.xmFilter
					+ "%' order by "
					+ this.orderColName
					+ " limit " + this.count + " offset " + this.start;
		else
			return "select bmxxid,ksxm,jg,ksxb,shfzh,zhkzhid,zongfen,zxmc,kskl,sflq,lqzy,sfjbmf from bmxx where shhqk=1 order by "
					+ this.orderColName
					+ " limit "
					+ this.count
					+ " offset "
	...
	public String getExamResult() {
		if (xmFilter.length() > 0)
			return "select bmxxid,ksxm,jg,ksxb,shfzh,zhkzhid,zongfen,zxmc,kskl,sflq,lqzy from bmxx where shhqk=1 and zhkzhid is not null and ksxm like '%"
					+ this.xmFilter
					+ "%' order by "
					+ this.orderColName
					+ " limit " + this.count + " offset " + this.start;
		else
			return "select bmxxid,ksxm,jg,ksxb,shfzh,zhkzhid,zongfen,zxmc,kskl,sflq,lqzy from bmxx where shhqk=1 and zhkzhid is not null order by "
					+ this.orderColName
					+ " limit "
					+ this.count
					+ " offset "
	...
	public String getAdmitResult(int score) {
		if (xmFilter.length() > 0)
			return "select bmxxid,ksxm,ksxb,shfzh,zhkzhid,zongfen,zxmc,kskl,sfjbmf,sflq,lqzy,sqly from bmxx where shhqk=1 and zhkzhid is not null and zongfen>="
					+ score
					+ " and ksxm like '%"
					+ this.xmFilter
					+ "%' order by "
	...
	public String admit(String bmxxs, boolean pass) {
		if (pass) {
			return "update bmxx, (select a.zyid,a.bmxxid,b.zymc from bkzy a, zhshzy b where a.zyid=b.zyid and a.xh=1) temp "
					+ " set bmxx.sflq=1, bmxx.lqzy=temp.zymc where bmxx.bmxxid=temp.bmxxid and bmxx.bmxxid in "
					+ bmxxs;
		} else {
	...
	public String update(Object obj) {
		...
		/*
		 * DBOperator dbo = new DBOperator(); ResultSet rs; Connection conn =
		 * null;
		 * 
		 * try{ dbo.init(false); conn = dbo.getConnection(); }catch(Exception
		 * e){ logger.error(e.getMessage());
		 * response.sendRedirect("/error.jsp?error=" + new
		 * UTF8String("数据库访问错误！").toUTF8String()); return; } try {
		 * PreparedStatement ps = conn.createPreparedStatement();
		 * }catch(Exception e) { logger.error(e.getMessage());
		 * response.sendRedirect("/error.jsp?error=" + new
		 * UTF8String("没有该考生或数据发生错误！").toUTF8String()); return; }finally {
