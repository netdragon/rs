...
public class ResetPwdCheckAction extends javax.servlet.http.HttpServlet
		implements javax.servlet.Servlet {
	protected static final LogHandler logger = LogHandler
			.getInstance(ResetPwdCheckAction.class);
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			String answer = BaseFunction.null2value(request
					.getParameter("pwdanswer"));
			String name = BaseFunction.null2value(request
					.getParameter("lgname"));
			...
			try {
				...
				if (!question.equals(user.getQuestion())
					...
					response.sendRedirect("error.jsp?error="
							+ new UTF8String("用户不存在或输入的信息不正确，请检查后再试！")
									.toUTF8String());
					return;
				}
