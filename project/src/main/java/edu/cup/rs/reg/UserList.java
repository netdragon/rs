...
public class UserList implements ICommonList {
	...
	public String getQL() {
		if (0 == this.userId) {
			if (userName.length() > 0)
				return "select userpwd,userid,username,email,question,answer,admin from user where username='"
						+ userName + "'";
			return "select userpwd,userid,username,email,question,answer,admin from user";
		}
		return "select userpwd,userid,username,email,question,answer,admin from user where userid="
				+ this.userId;
	}
	...
	public String modifyReg(User user) {
		return "update user set email='" + user.getEmail() + "', question='"
				+ user.getQuestion() + "', answer='" + user.getAnswer()
	...
	public String insert(Object obj) {
		...
		StringBuffer sql = new StringBuffer(
				"insert user(reg_time, username,userpwd,reg_ip,question,answer,email) values(now(),'");
