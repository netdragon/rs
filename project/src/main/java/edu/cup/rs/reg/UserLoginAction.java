...
public class UserLoginAction extends javax.servlet.http.HttpServlet implements
		javax.servlet.Servlet {
	protected static final LogHandler logger = LogHandler
			.getInstance(UserLoginAction.class);
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			String userPwd = BaseFunction.null2value(request
					.getParameter("txtpwd"));
			if (0 == userName.length()) {
				response.sendRedirect("error.jsp?error="
						+ new UTF8String("请输入正确的用户名！").toUTF8String());
				return;
			}
			String rand = (String) session.getAttribute("rand");
			String input = BaseFunction.null2value(request
			...
			try {
				...
				if (al.size() == 1) {
					...
					session.setAttribute("bmxxid",
							new String("" + bmxx.getBmxxid()));
				} else {
			...
			if (null != s_SrcUrl && s_SrcUrl.length() > 0) {
				...
				if (isAdmin == 1)
					response.sendRedirect(response
							.encodeRedirectURL("admin/adminindex.jsp?"
									+ timestamp));
				else
					response.sendRedirect(response
							.encodeRedirectURL("loginin/stumain.jsp?"
									+ timestamp));
			}
			return;
		} catch (Exception e) {
