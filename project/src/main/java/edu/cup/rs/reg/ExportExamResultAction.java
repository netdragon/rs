...
public class ExportExamResultAction extends BaseAction {
	protected static final LogHandler logger = LogHandler
			.getInstance(ExportExamResultAction.class);
	...
	static {
		...
		TEMPLATE = hm_env.get("RS_HOME").trim()
				+ "\\data\\appfiles\\template\\chengji_template.xls";
	...
	protected void execute(HttpServletRequest request,
		...
		try {
			...
			response.setHeader("Content-disposition", "filename=ks_chengji_"
					+ tempName + ".xls");
	...
	public static void saveExcel(String template, String fName)
		...
		try {
			...
			for (int i = 0; i < kmLen; i++) {
				...
				columnValue = ((null == columnValue) || (columnValue.length() == 0)) ? " "
						: columnValue;
