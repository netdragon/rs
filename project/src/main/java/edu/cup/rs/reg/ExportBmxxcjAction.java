...
public class ExportBmxxcjAction extends BaseAction {
	protected static final LogHandler logger = LogHandler
			.getInstance(ExportBmxxcjAction.class);
	...
	static {
		...
		TEMPLATE = hm_env.get("RS_HOME").trim()
				+ "\\data\\appfiles\\template\\bmxxcj_template.xls";
	...
	public static void saveExcel(String template, String fName)
		...
		try {
			...
			while (rs.next()) {
				...
				k++;
				rs_cjjd = dbo
						.query("select b.zxmc,zxjb,zxtxdz,zxybm,zxlxdh,njfzr,gysyw,gyssx,gyswy,gyswl,gyshx,gyssw,gysls,gyszz,gysdl,gysty,gyszf,"
								...
								+ "from cjjd a left join bmxx b on a.bmxxid=b.bmxxid where a.bmxxid="
								+ bmxxid);
