...
public class SaveStaticPageAction extends BaseAction {
	...
	protected static final LogHandler logger = LogHandler
			.getInstance(SaveStaticPageAction.class);
	...
	protected void execute(HttpServletRequest request,
		...
		try {
			...
			String content = BaseFunction.null2value(request
					.getParameter("content"));
			String edit_file = BaseFunction.null2value(request
					.getParameter("edit_file"));
			OutputStreamWriter out = null;
			try {
				if (0 < edit_file.length()) {
					String root = request.getRealPath("/");
					out = new OutputStreamWriter(new FileOutputStream(root
							+ "/admin/" + edit_file), "UTF-8");
			...
			response.sendRedirect("admin/edit_static_page.jsp?curr_file="
					+ edit_file);
			return;
		} catch (Exception e) {
