...
public class LogsList implements ICommonList {
	...
	public String getCount() {
		if (this.xmFilter.length() == 0)
			...
			return "select count(id) as count from sys_log where content like '%"
					+ this.xmFilter + "%'";
	}
	...
	public String getQL() {
		String sql;
		if (this.xmFilter.length() == 0)
			sql = "select * from sys_log order by timestamp desc limit "
					+ this.count + " offset " + this.start;
		else
			sql = "select * from sys_log where content like '%" + this.xmFilter
					+ "%' order by timestamp desc limit " + this.count
					+ " offset " + this.start;
		return sql;
	}
